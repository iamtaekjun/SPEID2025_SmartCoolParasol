...
#define BUZZER_PIN 10    // 부저 (Pin 10으로 변경)
...
// ============= 2. 객체 생성: DHT dht(...), Servo parasolServo =============
DHT dht(DHT_PIN, DHT_TYPE);
Servo parasolServo;

// ============= 3. 전역 상태: SensorData, SystemStatus 구조체와 임계값 상수 =============
// 센서 값 저장 구조체
struct SensorData {
    float temperature;
    float humidity;
...
// ============= 4. 함수 전방 선언부: 초기화/센싱/로직/액추에이터/출력 =============
...
// ============= 6. 초기화 계열 구현: initializeSystem/Pins/Sensors/Actuators, 하드웨어 테스트 =============
void initializeSystem() {
...
void initializeSensors() {
    ...
    if (!isnan(testTemp) && !isnan(testHum)) {
        Serial.println("  ✓ DHT22 센서 정상 작동");
        sensors.isValid = true;
    } else {
        Serial.println("  ⚠ DHT22 센서 오류 - 연결 상태 확인 필요");
    }
...
// ============= 7. 런타임 동작: readAllSensors → executeBasicLogic → (rain/heat/water) 결정 =============
void readAllSensors() {
    // DHT22 온습도 센서
...
// ============= 8. 상태/액추에이터 제어: deployParasol(), retractParasol() (선언돼 있고, 전개·수납 호출은 로직 내부에서 수행) =============
void deployParasol() {
...
// ============= 9. 상태 출력: 3초마다 printSystemStatus() 호출 =============
void printSystemStatus() {
